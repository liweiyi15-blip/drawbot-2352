...
# ================= 🛡️ V28.6 Scoring System =================
def get_signal_score(s, regime="TREND"):
    s = s.strip()
    
    # --- A. Bottom Radar (0 points tip) ---
    if "💡" in s: return 0.0 
...
def analyze_daily_signals(ticker):
    ...
    # 3. Momentum (Fixed: Valid Breakout)
...
async def list_stocks(interaction: discord.Interaction):
    ...
    for i, (price, signals, regime) in enumerate(results):
        ...
        score, _, reasons, has_bottom = generate_report_content(signals, regime)
...
@bot.tree.command(name="scores", description="查看V28评分表")
async def show_scores(interaction: discord.Interaction):
    embed = discord.Embed(title="📊 V28.6 机构完美闭环评分表", description="系统完全体。引入 **通道有效性校验**，杜绝缩量假突破。", color=discord.Color.gold())
...
`+2.0` 爆量抢筹 (阳) / 黄金坑 / 九转底
`-2.0` 爆量出货 (阴) / 九转顶
`+1.5` Supertrend多 / 主力吸筹 / 有效突破
`-1.5` Supertrend空 / 主力派发 / 有效跌破
""", inline=False)
